...
public class crawler2 extends JFrame {
	...
	public crawler2() throws IOException, URISyntaxException { //GUI
        ...
        String [][]b = {{"서울","경기","인천","대전","세종"},
                        {"충남","충북","광주","전남","전북"},
                        {"대구","경북","부산","울산","경남"},
                        {"강원","제주","전국","지역정보창"}}; // 2차원 배열을 이용한 JFrame을 생성해서 지역정보창을 만들어줌
		...
		inputbtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					e1.printStackTrace(); // 에러가 발생하면 단계별로 에러를 출력하는 메소드
					}
				}
		});
		...
		resetbtn.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				ta.setText(null); // 텍스트에어리어를 초기화 하는 메소드
	...
	private void submitAction() throws IOException {
       ...
       Document doc = Jsoup.connect(URL + realgetParameter(KEY_WORD, PAGE, localNumber)).get(); // URL 불러오기 (doc 객체에는 파싱한 HTML 코드가 들어감)
		...
		Elements elements = doc.select(".post"); // 클래스 post의 정보를 담아줌
